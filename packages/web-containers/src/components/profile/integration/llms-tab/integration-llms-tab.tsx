...
export default function IntegrationLLMsTab({
  tenantKey,
}: {
  tenantKey: string;
}) {
  // Fetch masked LLM credentials from API
  const {
    data: maskedCredentials = [],
    isLoading: isMaskedCredentialsLoading,
    refetch,
  } = useGetMaskedLLMCredentialsQuery({ org: tenantKey });
  ...
  const handleDeleteCredential = async (
    name: string,
    platformKey: string,
    isIntegrationCredential: boolean
  ) => {
    try {
      if (isIntegrationCredential) {
        await deleteIntegrationCredential({
          org: tenantKey,
          platform_key: platformKey,
          name,
        }).unwrap();
        toast.success("Integration credential deleted successfully");
      } else {
    ...
    <Card style={{ borderColor: "oklch(.922 0 0)" }}>
      <CardHeader>
        <div className="flex items-start gap-2">
          <CardDescription>
            These are LLM keys from third-party services that
                      ...
                      <TableRow key={cred.name + idx} style={{ borderColor: "oklch(.922 0 0)" }}>
                        ...
                        <TableCell>
                          <Button
                            ...
                            disabled={isDeletingIntegration || isDeletingCredential}
                            onClick={() =>
                              handleDeleteCredential(
                                cred.name,
                                cred.platform,
                                false // Since we're using masked credentials, we'll assume they're not integration credentials for now
                              )
                            }
                          >
